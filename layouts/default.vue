            ...
            <div class="w-60 min-h-full size-full m-6 flex flex-col" >
                <div class="w-[90%] bg-light-primary rounded-2xl self-center flex p-3 items-center">
                    ...
                    <div class="flex flex-col gap-1">
                        <h1 class="text-xs font-semibold text-secondary">Welcom {{ owner.owners[0].name}}</h1>
                ...
                <nav class="flex items-start flex-col justify-around mt-10 h-[300px]">
                    <NuxtLink class="flex items-center w-full py-3 pl-3 rounded-xl hover:bg-[var(--green-grace)]/80 transition-all duration-350 ease-in-out" :class=" Currentroute.path === route.path ? 'bg-red-500' : ''" v-for="(route, index) in Routes" :key="index" :to="route.path">
...
// functions
    ...
    const {data : owner} : any = useFetch('/api/Owner',{lazy: true}) ;
