        ...
        <UDrawer 
            v-model:open="open"
             direction="left"
              ...
              :handle="false" 
              :ui="{
                content: 'bg-[var(--deep-light-blue)] text-[var(--creamy-white)]', 
            }"
        >
            

            <template #content>
             <UButton class="absolute top-0 right-0" color="neutral" :class="['bg-transparent' ,'text-amber-50' ,'hover:bg-red-600']" variant="solid" icon="i-lucide-x" @click="open = false" />
  
            <div class="w-60 min-h-full size-full m-6 flex flex-col" >
                <div class="w-[90%] bg-primary rounded-2xl self-center flex p-3 items-center">
                    ...
                    <div class="flex flex-col gap-1">
                        <h1 class="text-xs font-semibold">Welcom Name</h1>
                        <p class="text-[10px] text-gray-400">Statue : {{ isAdmin? 'ADMIN' : 'Employee' }}</p>
                    </div>
                </div>
